...
{
public:
    /** @brief 刷屏实现: dev=面板, arg=本帧参数, timeout=超时 ms(0 = 驱动默认) */
    using FlushCallback = etl::inplace_function<int(struct device*, DeviceArg& arg, std::uint32_t timeout), kFlushCapacity>;

    /** @brief 只绑定 device, 窗口先留空由 SetWindow 填 (label 查不到时 device_ptr 为 nullptr) */
    ...
    /** @brief 设面板窗口(闭区间, 绝对坐标): 内部排序+校验; false = 非法, 成员保持原值 */
