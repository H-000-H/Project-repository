...
/** @brief RGB565: 16bpp, PARTIAL 分块刷 */
template <>
...
public:
    ...
    /** @brief 唯一入口: 首调构造单例, 之后再调只取用; label=DTS label, occupy=整屏分几块刷 */
    ...
    /** @brief 拉起整块屏: 开设备 + 要窗口 + 建 display + 挂回调 + 交缓冲; 幂等, 失败回滚 */
    int Init()
    {
        if (disp != nullptr)
        {
    ...
    /** @brief LVGL flush 入口(Init 里挂上): 结尾必须 flush_ready, 否则界面卡死 */
...
private:
    ...
    /** @brief 开设备 + 向驱动要窗口 (只给 Init 用, 免得出现"开了但没建屏"的半状态) */
    int Open()
    {
        if (opened)
        {
