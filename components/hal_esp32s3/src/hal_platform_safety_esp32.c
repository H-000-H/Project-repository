...
/*
 * ESP32-S3 安全停机垫底 — 全屏蔽策略下 (mini_tree ESP 构建不编 weak stub) 的
 * strong 占位。safe_state.c 的 enter_safe_state 会调用:
 *   hal_platform_critical_hardware_lock — 返回 VFS_OK 视为无硬件锁需求 (ESP 靠
...
int hal_platform_critical_hardware_lock(void) { return VFS_OK; }

int hal_pwm_force_stop_all(void) { return VFS_OK; }
