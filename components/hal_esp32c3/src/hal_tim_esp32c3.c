 ...
 * @brief ESP32-C3 TIM HAL — LEDC 作 PWM/OC 后端
 *
 * DTS 映射:
 *   hw-instance   → LEDC timer 编号 (C3: 4 个 timer)
 *   clk-periph    → PWM 频率 Hz
 *   autoreload    → 占空比分辨率上限 (如 1023 → 10-bit)
 *   ocN-pin[1]    → GPIO
 *   ocN-channel-id→ LEDC channel (C3: 6 通道, 仅低速模式)
 */
#include "hal_tim.h"
#include "status.h"
...
int hal_tim_device_init(hal_tim_device* pdev, hal_tim_platform_unique_config* unique, hal_tim_host_config* host)
{
    ...
    for (i = 0; i < HAL_TIM_ESP_MAX; i++)
    {
        if (!s_tim_rt[i].used)
        {
            MINI_MEM_SET(&s_tim_rt[i], 0, sizeof(s_tim_rt[i]));
            s_tim_rt[i].used = 1;
...
int hal_tim_open(hal_tim_device* pdev)
{
    ...
    MINI_MEM_SET(&tcfg, 0, sizeof(tcfg));
    ...
    MINI_MEM_SET(&ccfg, 0, sizeof(ccfg));
...
int hal_tim_close(hal_tim_device* pdev)
{
    ...
    if (!rt)
        return MINI_ERR_INVAL;
    MINI_IGNORE_RESULT(ledc_stop(rt->speed_mode, rt->channel, 0));
    return MINI_OK;
}
...
int hal_tim_pwm_update(hal_tim_device* pdev, uint32_t channel, uint32_t frequency, uint32_t duty)
{
    ...
    MINI_IGNORE_RESULT(channel);
    MINI_IGNORE_RESULT(frequency);
    if (!rt)
        return MINI_ERR_INVAL;
...
int hal_tim_interrupt_config(hal_tim_device* pdev, uint32_t interrupt_config)
{
    ...
    MINI_IGNORE_RESULT(interrupt_config);
    return MINI_ERR_NOTSUPP;
}
...
{
    MINI_IGNORE_RESULT(pdev);
    MINI_IGNORE_RESULT(encoder_mode);
    return MINI_ERR_NOTSUPP;
}
