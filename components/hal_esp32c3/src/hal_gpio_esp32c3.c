 ...
 * @brief ESP32-C3 GPIO 硬件直投实现
 * @note 设计理念：尽量使用 ESP-IDF 高层 API，减少代码量，提高可读性，不是不得已不使用寄存器操作
 * @note ESP32-C3 无端口基地址/时钟概念, port/clk_bus 不使用, pin 承载 SoC GPIO 编号 (GPIO0..21)
...
{
    MINI_IGNORE_RESULT(pdev);
    MINI_IGNORE_RESULT(output_type);
    return MINI_ERR_NOTSUPP;
}
