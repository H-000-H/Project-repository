...
/*
 * I2C HAL — ESP32-C3 实现 (Master)
 *
 * ESP 适配: cfg.i2c = i2c_port_t (C3 仅 1 个 I2C 控制器); scl/sda.pin = SoC GPIO;
 *           port/clk/af 不使用
...
MINI_STATIC_INLINE int esp_i2c_map_err(esp_err_t err)
{
    if (err == ESP_OK)
        return MINI_OK;
...
MINI_STATIC_INLINE int esp_i2c_timeout(uint32_t timeout_ms)
{
...
int hal_i2c_bus_host_deinit(struct hal_i2c_bus_host* host)
{
    ...
    {
        MINI_IGNORE_RESULT(i2c_del_master_bus(hctx->bus));
        MINI_MEM_SET(hctx, 0, sizeof(*hctx));
    }
...
int hal_i2c_dev_hw_open(struct hal_i2c_dev* dev)
{
    ...
    if (!hctx->used)
    {
        MINI_MEM_SET(&bus_cfg, 0, sizeof(bus_cfg));
    ...
    MINI_MEM_SET(&dev_cfg, 0, sizeof(dev_cfg));
    ...
    if (err != ESP_OK)
    {
        MINI_MEM_SET(dctx, 0, sizeof(*dctx));
        return esp_i2c_map_err(err);
    }
...
int hal_i2c_dev_hw_close(struct hal_i2c_dev* pdev)
{
    ...
    if (dctx && dctx->handle)
    {
        MINI_IGNORE_RESULT(i2c_master_bus_rm_device(dctx->handle));
        MINI_MEM_SET(dctx, 0, sizeof(*dctx));
    }
    pdev->hw_open = 0;
    return MINI_OK;
