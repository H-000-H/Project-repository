 ...
 * ESP32-C3 适配: cfg.uart 承载 uart_port_t (C3 仅 UART0/UART1); tx/rx.pin 为 SoC
 * GPIO 编号; port/clk/af 不使用。DMA 不支持 (无 STM32/WCH 风格 DMA API), 返回
 * MINI_ERR_NOTSUPP。
 */
#include "hal_uart.h"
#include "status.h"
...
MINI_STATIC_INLINE uint32_t esp32_uart_timeout_ms(uint32_t timeout_ms)
{
...
int hal_uart_dev_hw_open(struct hal_uart_bus_host* host)
{
    ...
    MINI_MEM_SET(&idf_cfg, 0, sizeof(idf_cfg));
