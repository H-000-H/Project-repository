...
/*
 * CAN HAL — ESP32-C3 TWAI (新 esp_twai API)
 *
 * ESP 适配: cfg.can = 控制器索引 (C3 仅 1 个 TWAI 控制器); tx/rx.pin = GPIO;
...
MINI_STATIC_INLINE int esp_can_map_err(esp_err_t err)
{
    if (err == ESP_OK)
        return MINI_OK;
...
static bool esp_can_on_rx_done(twai_node_handle_t handle, const twai_rx_done_event_data_t* edata, void* user_ctx)
{
    ...
    MINI_MEM_SET(&rx, 0, sizeof(rx));
    MINI_MEM_SET(&item, 0, sizeof(item));
    rx.buffer     = buf;
...
int hal_can_bus_host_deinit(struct hal_can_bus_host* host)
{
    ...
    if (ctx && ctx->used)
    {
        if (ctx->node)
        {
            MINI_IGNORE_RESULT(twai_node_disable(ctx->node));
            MINI_IGNORE_RESULT(twai_node_delete(ctx->node));
        }
        if (ctx->rx_q)
...
int hal_can_dev_hw_open(struct hal_can_dev* dev)
{
    ...
    if (!ctx->used)
    {
        MINI_MEM_SET(&ncfg, 0, sizeof(ncfg));
...
int hal_can_filter_config(struct hal_can_bus_host* host, const struct hal_can_filter_config* filter)
{
    ...
    MINI_MEM_SET(&mcfg, 0, sizeof(mcfg));
    mcfg.id     = filter->id;
    ...
    MINI_IGNORE_RESULT(twai_node_enable(ctx->node));
    return esp_can_map_err(err);
}
