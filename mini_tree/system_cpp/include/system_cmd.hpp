...
class CmdFn
{
    ...
    struct Vtable
    {
        bool (*invoke)(void* self, const void* arg, size_t len,
                       void* ctx); /**< 调用包装的可调用对象 */
        void (*copy)(void* dst, const void* src); /**< 拷贝构造 */
        void (*destroy)(void* self); /**< 析构销毁 */
    };

    template <typename F> static bool invoke_fn(void* self, const void* arg, size_t len, void* ctx)
    {
        return (*static_cast<F*>(self))(arg, len, ctx);
    }

    template <typename F> static void copy_fn(void* dst, const void* src)
    {
        new (dst) F(*static_cast<const F*>(src));
    }

    template <typename F> static void destroy_fn(void* self) { static_cast<F*>(self)->~F(); }

    const Vtable* m_vtable; /**< 当前存储对象的虚表指针 (nullptr = 空) */
    union Storage
    {
        char data[StorageSz - sizeof(const Vtable*)];
        max_align_t align_;
    } m_storage; /**< SBO 内联存储区 */

    static_assert(StorageSz > sizeof(const Vtable*), "StorageSz must be larger than pointer size");

public:
    CmdFn() : m_vtable(nullptr) {}

    template <typename F, typename = etl::enable_if_t<!etl::is_same_v<etl::decay_t<F>, CmdFn>>>
    CmdFn(F&& f) : m_vtable(&k_vtable<etl::decay_t<F>>)
    {
    ...
    {
        if (m_vtable)
            m_vtable->copy(m_storage.data, other.m_storage.data);
    }
...
private:
    template <typename F> static const Vtable k_vtable;
};
...
{
public:
    static constexpr size_t k_max_cmd_name_len =
        SYS_CMD_MAX_NAME_LEN; /**< 命令名最大长度 (含 '\0') */
    ...
    template <typename T> static TypeIdToken get_type_id()
    {
