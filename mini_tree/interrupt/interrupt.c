...
void interrupt_virtual_dispatch(uint16_t virq_num)
{
    ...
    if (top)
    {
        ...
        if (need_bottom_half && work)
            MINI_IGNORE_RESULT(interrupt_bottom_half_submit(work));
    }
    else if (work)
    {
        MINI_IGNORE_RESULT(interrupt_bottom_half_submit(work));
...
void bottom_half_run_pending(struct fifo_spsc* fifo)
{
    ...
    while (fifo_read_data(fifo, &elem) == BUFF_OK)
    {
        ...
        MINI_ATOMIC_STORE(&work->executing, true, MINI_RELEASE);
        work->fn(work->arg);
        MINI_ATOMIC_STORE(&work->executing, false, MINI_RELEASE);

        MINI_ATOMIC_STORE(&work->pending, false, MINI_RELEASE);

        while (MINI_ATOMIC_EXCHANGE(&work->rerun, false, MINI_ACQ_REL))
        {
