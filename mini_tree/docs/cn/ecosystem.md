...
| 库 | 路径 | 版本 | 作用 | 接入方式 |
| :--- | :--- | :--- | :--- | :--- |
| LVGL | Fetch / `lib/lvgl` | master (本地优先) | 彩色 GUI | `mini_tree_link_lvgl` + `lv_conf.h` |
...
- **UI 胶水层（不在 mini_tree 里）**：LVGL / u8g2 的 flush 回调经 `<project>/ui/display/display_ui_bridge.h` 统一入口，走 `device_ioctl(DISPLAY_CMD_*)` 触显示硬件；不直调 `bus_*` / `hal_*`，换屏仅需更换 device 指针。中间件只提供 `mini_tree_link_lvgl` 这类链接工具，不感知 UI 层。
