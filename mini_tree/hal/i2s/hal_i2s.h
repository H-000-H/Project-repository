 ...
 */
mt_err_t hal_i2s_bus_host_init(struct hal_i2s_bus_host* host, int hw_idx, const struct hal_i2s_bus_config* cfg) MINI_WARN_UNUSED_RESULT;
/**
 * @brief 反初始化 I2S 总线主机, 释放硬件资源
 * @param[in] host I2S host 对象指针
 * @return 成功返回 MINI_OK, host 为空返回 MINI_ERR_INVAL
 */
mt_err_t hal_i2s_bus_host_deinit(struct hal_i2s_bus_host* host) MINI_WARN_UNUSED_RESULT;
/**
 ...
 */
mt_err_t hal_i2s_dev_init(struct hal_i2s_dev* pdev, struct hal_i2s_bus_host* host, const struct hal_i2s_device_config* cfg) MINI_WARN_UNUSED_RESULT;
/**
 * @brief 解绑 I2S 设备并复位状态
 * @param[in] pdev I2S 设备对象指针
 * @return 成功返回 MINI_OK, pdev 为空返回 MINI_ERR_INVAL
 */
mt_err_t hal_i2s_dev_deinit(struct hal_i2s_dev* pdev) MINI_WARN_UNUSED_RESULT;
/**
 * @brief 打开 I2S 设备硬件 (引用计数 +1)
 * @param[in] pdev I2S 设备对象指针
 * @return 成功返回 MINI_OK, pdev 为空返回 MINI_ERR_INVAL
 */
mt_err_t hal_i2s_dev_hw_open(struct hal_i2s_dev* pdev) MINI_WARN_UNUSED_RESULT;
/**
 * @brief 关闭 I2S 设备硬件 (引用计数 -1)
 * @param[in] pdev I2S 设备对象指针
 * @return 成功返回 MINI_OK, pdev 为空返回 MINI_ERR_INVAL
 */
mt_err_t hal_i2s_dev_hw_close(struct hal_i2s_dev* pdev) MINI_WARN_UNUSED_RESULT;
/**
 ...
 */
mt_err_t hal_i2s_sync(struct hal_i2s_dev* pdev, const uint16_t* tx, uint16_t* rx, size_t samples, uint32_t timeout_ms,
 ...
 */
mt_err_t hal_i2s_set_dma_irq_mode(struct hal_i2s_dev* pdev, uint32_t irq_mode) MINI_WARN_UNUSED_RESULT;
/**
 ...
 */
mt_err_t hal_i2s_get_dma_irq_mode(struct hal_i2s_dev* pdev, uint32_t* irq_mode) MINI_WARN_UNUSED_RESULT;
 ...
 */
mt_err_t hal_i2s_dma_circ_start(struct hal_i2s_dev* pdev, int tx_enable, int rx_enable) MINI_WARN_UNUSED_RESULT;
/**
 * @brief 停止 DMA 循环缓冲传输
 * @param[in] pdev I2S 设备对象指针
 * @return 成功返回 MINI_OK, pdev 为空返回 MINI_ERR_INVAL
 */
mt_err_t hal_i2s_dma_circ_stop(struct hal_i2s_dev* pdev) MINI_WARN_UNUSED_RESULT;
/**
 * @brief 向 DMA 循环 TX 缓冲写入采样
 * @param[in] pdev I2S 设备对象指针
 * @param[in] data 发送采样缓冲区
 * @param[in] samples 写入采样数 (16-bit)
 * @return 成功返回 MINI_OK, 缓冲满返回 MINI_ERR_NOMEM, 失败返回 MINI_ERR_*
 */
mt_err_t hal_i2s_dma_circ_write(struct hal_i2s_dev* pdev, const uint16_t* data, uint32_t samples) MINI_WARN_UNUSED_RESULT;
/**
 * @brief 从 DMA 循环 RX 缓冲读取采样
 * @param[in] pdev I2S 设备对象指针
 * @param[out] data 接收采样缓冲区
 * @param[in] samples 读取采样数 (16-bit)
 * @return 成功返回 MINI_OK, 数据不足返回 MINI_ERR_AGAIN, 失败返回 MINI_ERR_*
 */
mt_err_t hal_i2s_dma_circ_read(struct hal_i2s_dev* pdev, uint16_t* data, uint32_t samples) MINI_WARN_UNUSED_RESULT;
 ...
 */
mt_err_t hal_i2s_transfer_async(struct hal_i2s_dev* pdev, const uint16_t* tx, uint16_t* rx, size_t samples, hal_i2s_callback_t cb,
 ...
 */
mt_err_t hal_i2s_transfer_poll(struct hal_i2s_dev* pdev, uint32_t timeout_ms) MINI_WARN_UNUSED_RESULT;
