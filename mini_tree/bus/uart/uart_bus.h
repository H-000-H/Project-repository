 ...
 */
mt_err_t uart_bus_host_init(struct device* pdev, const struct hal_uart_config* cfg) MINI_WARN_UNUSED_RESULT;
/**
 * @brief UART host 反初始化 (ref_count > 0 时返回 BUSY)
 * @param[in] pdev controller device (host)
 * @return 成功返回 MINI_OK, BUSY 返回 MINI_ERR_BUSY, 失败返回 MINI_ERR_*
 */
mt_err_t uart_bus_host_deinit(struct device* pdev) MINI_WARN_UNUSED_RESULT;
 ...
 * @return 成功返回 MINI_OK, 失败返回 MINI_ERR_*
 */
mt_err_t uart_bus_client_register(struct device* pdev) MINI_WARN_UNUSED_RESULT;
/**
 ...
 */
mt_err_t uart_bus_open(struct device* pdev) MINI_WARN_UNUSED_RESULT;
/**
 * @brief 关闭 UART client (仅 IO gate, 不改 ref_count)
 * @param[in] pdev client device
 * @return 成功返回 MINI_OK, 失败返回 MINI_ERR_NODEV
 */
mt_err_t uart_bus_close(struct device* pdev) MINI_WARN_UNUSED_RESULT;
 ...
 * @return 成功返回 MINI_OK, 失败返回 MINI_ERR_*
 */
mt_err_t uart_bus_write(struct device* pdev, const uint8_t* data, size_t len, uint32_t timeout_ms) MINI_WARN_UNUSED_RESULT;
 ...
 * @param[out] len 读取长度
 * @param[in] timeout_ms 超时 (ms, 0=平台默认)
 * @return 成功返回已读字节数或 MINI_OK, 失败返回 MINI_ERR_*
 */
mt_err_t uart_bus_read(struct device* pdev, uint8_t* data, size_t len, uint32_t timeout_ms) MINI_WARN_UNUSED_RESULT;
 ...
 * @return 成功返回 MINI_OK (有读时返回已读字节数), 失败返回 MINI_ERR_*
 */
mt_err_t uart_bus_transfer(struct device* pdev, const uint8_t* tx, uint8_t* rx, size_t tx_len, size_t rx_len, uint32_t timeout_ms) MINI_WARN_UNUSED_RESULT;
...
#pragma GCC poison hal_uart_write hal_uart_read
