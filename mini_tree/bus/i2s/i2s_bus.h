 ...
 */
mt_err_t i2s_bus_host_init(struct device* pdev, const struct hal_i2s_bus_config* cfg) MINI_WARN_UNUSED_RESULT;
/**
 * @brief 反初始化 I2S 总线主机, 释放硬件资源
 * @param[in] pdev controller device (host)
 * @return 成功返回 MINI_OK, pdev 为空返回 MINI_ERR_INVAL
 */
mt_err_t i2s_bus_host_deinit(struct device* pdev) MINI_WARN_UNUSED_RESULT;
/**
 * @brief 查询 I2S 总线主机角色
 * @param[in] pdev controller device (host)
 ...
 * @return 成功返回 MINI_OK, 失败返回 MINI_ERR_*
 */
mt_err_t i2s_bus_client_register(struct device* pdev, const struct hal_i2s_device_config* cfg, struct i2s_bus_client** out) MINI_WARN_UNUSED_RESULT;
/**
 ...
 * @return 成功返回 MINI_OK, pdev 为空返回 MINI_ERR_INVAL
 */
mt_err_t i2s_bus_open(struct device* pdev) MINI_WARN_UNUSED_RESULT;
/**
 * @brief 关闭 I2S 设备 (引用计数 -1)
 * @param[in] pdev controller device (host)
 * @return 成功返回 MINI_OK, pdev 为空返回 MINI_ERR_INVAL
 */
mt_err_t i2s_bus_close(struct device* pdev) MINI_WARN_UNUSED_RESULT;
/**
 ...
 */
mt_err_t i2s_bus_transfer(struct device* pdev, const uint16_t* tx, uint16_t* rx, size_t samples, uint32_t timeout_ms,
 ...
 * @return 成功返回 MINI_OK, 失败返回 MINI_ERR_*
 */
mt_err_t i2s_bus_transfer_poll(struct device* pdev, uint32_t timeout_ms) MINI_WARN_UNUSED_RESULT;
 ...
 * @return 成功返回 MINI_OK, pdev 为空返回 MINI_ERR_INVAL
 */
mt_err_t i2s_bus_set_dma_irq_mode(struct device* pdev, uint32_t irq_mode) MINI_WARN_UNUSED_RESULT;
/**
 * @brief 获取 DMA 中断模式
 * @param[in] pdev controller device (host)
 ...
 */
mt_err_t i2s_bus_get_dma_irq_mode(struct device* pdev, uint32_t* irq_mode) MINI_WARN_UNUSED_RESULT;
 ...
 * @return 成功返回 MINI_OK, 失败返回 MINI_ERR_*
 */
mt_err_t i2s_bus_dma_circ_start(struct device* pdev, int tx_enable, int rx_enable) MINI_WARN_UNUSED_RESULT;
/**
 * @brief 停止 DMA 循环缓冲传输
 * @param[in] pdev controller device (host)
 * @return 成功返回 MINI_OK, pdev 为空返回 MINI_ERR_INVAL
 */
mt_err_t i2s_bus_dma_circ_stop(struct device* pdev) MINI_WARN_UNUSED_RESULT;
/**
 ...
 */
mt_err_t i2s_bus_dma_circ_write(struct device* pdev, const uint16_t* data, uint32_t samples) MINI_WARN_UNUSED_RESULT;
/**
 ...
 */
mt_err_t i2s_bus_dma_circ_read(struct device* pdev, uint16_t* data, uint32_t samples) MINI_WARN_UNUSED_RESULT;
