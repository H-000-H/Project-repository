...
/** @brief 通用通信底座 (无单例/无任务); 发送长度取 span.size(), 上限 k_buffer_size */
...
public:
    ...
    /** @brief 最近一次读到的数据视图 (指向接收缓冲, 长度为 get_rx_len()) */
    ...
    void send_log();      /**< 打印发送缓冲 (字符串形式) */
    void recv_log();      /**< 打印接收缓冲 (字符串形式) */
    void send_recv_log(); /**< 发送与接收缓冲一起打印 */
