 ...
 * @param[out] buf     uint8_t* 输出缓冲, 写入读到的字节
 * @param[in]  want    uint32_t 期望读取的字节数
...
static int ota_download_source(void* param, uint8_t* buf, uint32_t want, int* out_len)
{
    ...
    if (!s_ready_sent)
    {
        ...
        static const uint8_t k_ready_byte = 'R';
